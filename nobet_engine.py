...
#hiç nöbet tutmayan 1.8 verdim.
#1 nöbet tutanlara ise 1.9
#4 tutanlara ise 3.2 bu sayede öncelik mekanizmamız her daim uygun #çalışır.
#Ceyda ilhan,Glsah ve Büşra atayı ortalamaya yaklaştırıldı.
#Gazi,glsah,ibni sina 2 ortalamaya yaklaşıldı.
# ================== GEÇMİŞ NÖBET YÜKLERİ ==================
GECMIS_YUK = {
...
def tarih_sor(mesaj):
    while True:
        t = input(mesaj + " (yyyy.aa.gg): ").strip()
        try:
...
def zorunlu_secim(grup,d,w,tatil,totals,counts,weekday_stats,last_dates,bayram_stats):
    ...
    for p in grup:
        if p in eklenme_tarihi and d < eklenme_tarihi[p]: continue
        if p in cikma_tarihi and d >= cikma_tarihi[p]: continue
...
def main():
    ...
    # ECZANE EKLE/ÇIKAR
    if input("Eczane ekle/çıkar? (E/H): ").lower()=="e":
        while True:
            sil=input("Çıkarılacak eczane (boş=bitir): ")
            if not sil: break
            cikma_tarihi[sil]=tarih_sor(f"{sil} çıkış tarihi")

        while True:
            yeni=input("Yeni eczane (boş=bitir): ")
            if not yeni: break
            grp=input("Grubu: ").upper()
            eklenme_tarihi[yeni]=tarih_sor(f"{yeni} eklenme tarihi")
            groups[grp].append(yeni)
            totals[yeni]=sum(totals.values())/len(totals)
            counts[yeni]=0
            weekday_stats[yeni]={i:0 for i in range(7)}
            bayram_stats[yeni]=0
            gecmis_katsayi_map[yeni]=0
    ...
    summary.append([
        "Eczane","Grup","Geçmiş Katsayı","Geçmiş Bayram","Toplam Nöbet","Toplam Katsayı","Bayram",
    ...
    wb.remove(wb["Sheet"])
    wb.save("Son.xlsx")
    print("✅ Plan hazır — tüm kurallar aktif")

    # 🔵 AYLIK DETAY EXCEL (main içine alındı)
    wb2 = Workbook()
    ws2 = wb2.active
    ...
    print("📊 Aylık nöbet detay dosyası oluşturuldu")

def run_schedule(2026, 4, 6):
    main(y,m,nm)
